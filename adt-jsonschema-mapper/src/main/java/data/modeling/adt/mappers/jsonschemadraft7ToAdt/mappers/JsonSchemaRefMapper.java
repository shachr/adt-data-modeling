...
public class JsonSchemaRefMapper extends JsonSchemaMapper<ReferenceObjectType> {
    ...
    @Override
    public ReferenceObjectType toAdt(Map<String, Object> value) throws AdtException {
        // todo: add logic
        ...
        ReferenceObjectType referenceObjectType = new ReferenceObjectType(ref);
        this.getJsonSchemaReferences().addReference(referenceObjectType);

        // read definition
        Map definitions = (Map)this.jsonSchemaDoc.get("definitions");
        Map definition = (Map)definitions.get(definitionName);
        NamedType namedType = NamedType.of(referenceObjectType.getReferenceName(), this.adtMapperRegistry.toAdt(definition));
